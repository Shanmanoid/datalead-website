...
import { ArrowLeft, CheckCircle, AlertTriangle, Lightbulb, ChevronDown, Database, Layers, Zap, Network, Code, BarChart3, Shield, Box, Server, Settings, GitBranch, Cpu } from 'lucide-react'
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            <ScrollReveal direction="left">
              <div className="h-full rounded-2xl bg-surface-elevated border border-border border-l-4 border-l-accent p-8 hover:shadow-xl transition-shadow duration-300">
            ...
            <ScrollReveal direction="right">
              <div className="h-full rounded-2xl bg-surface-elevated border border-border border-l-4 border-l-brand p-8 hover:shadow-xl transition-shadow duration-300">
                <div className="flex items-center gap-3 mb-5">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 max-w-3xl mx-auto">
            {features.map((featureKey, i) => (
              <ScrollReveal key={i} delay={i * 0.08}>
                <div className="flex items-start gap-3 p-4 rounded-xl bg-surface-elevated border border-border hover:border-brand/30 hover:shadow-md transition-all duration-300">
                  <CheckCircle size={20} className="text-brand mt-0.5 shrink-0" />
                  <span className="text-text-primary">{t(featureKey)}</span>
                </div>
              </ScrollReveal>
            ))}
          </div>
              ...
              return (
                <ScrollReveal key={tech} delay={i * 0.1}>
                  <motion.div
                    ...
                    className="group relative flex flex-col items-center justify-center w-32 h-32 rounded-2xl bg-surface-elevated border border-border hover:border-brand/30 hover:shadow-xl transition-all duration-300 overflow-hidden"
                  >
                    <div className={`absolute top-0 left-0 right-0 h-1 bg-gradient-to-r ${meta.gradient}`} />
                    <div className={`w-12 h-12 rounded-xl bg-gradient-to-br ${meta.gradient} flex items-center justify-center mb-3 shadow-md group-hover:scale-110 transition-transform`}>
                      <Icon size={24} className="text-white" />
                    </div>
                    <span className="text-sm font-semibold text-text-primary leading-tight text-center px-2">
                      {tech}
                    </span>
                  </motion.div>
                </ScrollReveal>
